...
function fillTaskAndTopicSelects(){
 document.getElementById("note-task").innerHTML='<option value="">-- No specific task --</option>'+selectOptions(tasks,item=>`${item.title}${item.active?"":" (Archived)"}`);
 document.getElementById("note-topic").innerHTML='<option value="">-- No specific topic --</option>'+selectOptions(topics,item=>item.topic_name);
}
...
async function loadReferenceData(){
 const[{data:taskData,error:taskError},{data:topicData,error:topicError}]=await Promise.all([
  db.from("assessments").select("id,title,topic_id,active").eq("class_id",pupil.class_id).order("created_at",{ascending:false}),
...
async function loadNotes(){
 const{data,error}=await db.from("pupil_action_points").select("*,assessments(title),topics(topic_name)").eq("pupil_id",user.id).order("created_at",{ascending:false});
 ...
 document.getElementById("notes-body").innerHTML=notes.map(item=>`<tr class="${item.actioned?"actioned-row":""}"><td>${escapeHtml(noteTypes[item.note_type])}</td><td>${escapeHtml(item.assessments?.title||"–")}</td><td>${escapeHtml(item.topics?.topic_name||"–")}</td><td class="note-comment">${escapeHtml(item.revision_comment)}</td><td><input class="action-checkbox" type="checkbox" aria-label="Mark action point as completed" ${item.actioned?"checked":""} onchange="setActioned('${item.id}',this.checked)"></td><td><button class="secondary" type="button" onclick="editNote('${item.id}')">Edit</button> <button class="danger" type="button" onclick="deleteNote('${item.id}')">Delete</button></td></tr>`).join("")||"<tr><td colspan='6'>No notes or action points yet.</td></tr>";
}
document.getElementById("note-form").addEventListener("submit",async event=>{
 event.preventDefault();
 const row={pupil_id:user.id,note_type:document.getElementById("note-type").value,assessment_id:document.getElementById("note-task").value||null,topic_id:document.getElementById("note-topic").value||null,revision_comment:document.getElementById("note-comment").value.trim(),actioned:document.getElementById("note-actioned").checked};
...
function editNote(id){const item=notes.find(note=>note.id===id);if(!item)return;editingNoteId=id;document.getElementById("note-type").value=item.note_type;document.getElementById("note-task").value=item.assessment_id||"";document.getElementById("note-topic").value=item.topic_id||"";document.getElementById("note-comment").value=item.revision_comment;document.getElementById("note-actioned").checked=item.actioned;document.getElementById("note-form-title").textContent="Edit action point";document.getElementById("note-submit").textContent="Save changes";document.getElementById("cancel-note-edit").classList.remove("hidden");document.getElementById("note-form").scrollIntoView({behavior:"smooth",block:"start"});}
